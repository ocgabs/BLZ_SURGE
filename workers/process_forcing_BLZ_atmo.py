...
Processes GRIB filter files as specified in the YAML Nowcast file, converts
...
def hourssince1900(config, arg, dir, model_run, t, i):
    filename = dir["grib_dir"]+arg["file_template"]
...
def ModelRun(dir):
    list_of_files = glob.glob(dir["grib_dir"]+'/*.grb2')
    latest_file = max(list_of_files, key=os.path.getctime)
    hour = latest_file[-24:-22]
    #hour = '00'
    return hour
...
def createNetCDF(lats, lons, dir, C, arg):
    ...
    if C == 0:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var1"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'w', format = 'NETCDF3_CLASSIC')
    ...
    if C == 1:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var2"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'w', format = 'NETCDF3_CLASSIC')
    ...
    if C == 2:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var3"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'w', format = 'NETCDF3_CLASSIC')
...
def loaddataNetCDF(config, arg, dir, model_run, t, i, C):
    ...
    if C == 0:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var1"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'a')
        ...
        grb = grbs.select(name=arg["var1_des"])
        Date_1900 = datetime.datetime(1900, 1, 1, 0, 0, 0, 0)
        #extract the data
        for i in range(int(arg["forecast_hrs"])):
            var = grb[i].values
            ...
            DateTime = datetime.datetime(grb[i].validDate.year, grb[i].validDate.month, grb[i].validDate.day, grb[i].validDate.hour, 0, 0, 0)
            diff = DateTime - Date_1900
            days, seconds = diff.days, diff.seconds
            hours = days * 24 + (seconds // 3600)
    ...
    if C == 1:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var2"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'a')

        filename = dir["grib_dir"]+arg["file_template"]
        #filename = '/Volumes/projectsa/C_RISC/TEST/gfs.t06z.pgrb2.0p25.f'+str(t[i])
        filename = filename.format(**arg)
        grbs = pygrib.open(filename)
        grb = grbs.select(name=arg["var2_des"])
        Date_1900 = datetime.datetime(1900, 1, 1, 0, 0, 0, 0)
        for i in range(int(arg["forecast_hrs"])):
            var = grb[i].values
            var = np.flip(var,0)
    ...
    if C == 2:
        ...
        force = nc.Dataset(dir["netcdf_dest_dir"]+dir["netcdf_name"]+'-'+arg["var3"]+'-y'+ymd[0:4]+'m'+ymd[5:7]+'d'+ymd[8:10]+'h'+arg['model_run']+'.nc', 'a')

        filename = dir["grib_dir"]+arg["file_template"]
        #filename = '/Volumes/projectsa/C_RISC/TEST/gfs.t06z.pgrb2.0p25.f'+str(t[i])
        filename = filename.format(**arg)
        grbs = pygrib.open(filename)
        grb = grbs.select(name=arg["var3_des"])
        Date_1900 = datetime.datetime(1900, 1, 1, 0, 0, 0, 0)
        for i in range(int(arg["forecast_hrs"])):
            var = grb[i].values
            var = np.flip(var,0)
...
def delete_old_netcdf(dir):
    ...
    filelist = glob.glob(dir['netcdf_dest_dir']+'*.nc')
